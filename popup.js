...
document.addEventListener('DOMContentLoaded', () => {
  ...
  dropdownButton.addEventListener('click', () => {
    ...
    if (isActive) {
      dropdownButton.textContent = '请选择期刊范围（点击收回）';
    } else {
      dropdownButton.textContent = '请选择期刊范围（点击展开）';
    }
  });
...
function generateAndCopyLink() {
  chrome.storage.local.get(['journalSources'], (data) => {
    ...
    if (selectedCategories.length > 0) {
      const journalFilters = selectedCategories.map(cat =>
        journalSources[cat].map(j => `source:"${j}"`).join(' OR ')
      ).join(' OR ');
      query = query ? `${query} (${journalFilters})` : `(${journalFilters})`;
    }
